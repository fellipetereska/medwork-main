...
function GridInventario({
  ...
  const findUnidade = (item) => {
    try {
      if (!item) {
        return 'N/A';
      }
      const unidades = unidade.find((c) => c.id_unidade === item);
  ...
  const findSetor = (item) => {
    try {
      ...
      const setores = setor.find((c) => c.id_setor === item)
      return setores ? setores.nome_setor : 'N/A'
    } catch (error) {
      console.log("Erro ao buscar setor!", error)
    }
  }

  const findProcesso = (item) => {
    try {
      if (!item) {
        return 'N/A'
      }
      const processos = processo.find((c) => c.id_processo === item)
      return processos ? processos.descricao : 'N/A'
    } catch (error) {
  ...
  const findRisco = (item) => {
    try {
      ...
      const riscos = risco.find((c) => c.id_risco === item)
      return riscos ? riscos.nome_risco : 'N/A'
    } catch (error) {
      console.log("Erro ao buscar Risco!", error)
    }
  }
      ...
      <div className="w-full">
        <div className="w-11/12 mx-auto relative overflow-x-auto shadow-md sm:rounded-lg">
          <table className="min-w-full table-auto text-sm text-left rtl:text-right text-gray-500">
            ...
            <tbody>
              {inventario && inventario.map((item, i) => (
                <tr
                  key={i}
